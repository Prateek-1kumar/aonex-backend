...
function ctxGet(c: { get: (k: string) => string | undefined; env: Record<string, unknown> }, key: string): string {
  return (c.get(key) ?? (c.env as Record<string, unknown>)?.[key]) as string;
}
...
export function ebayRoutes(deps: EbayRouteDeps): Hono {
  ...
  app.post('/connect', async (c) => {
    ...
    const tenantId = TenantId.unsafeFrom(ctxGet(c as any, 'tenantId'));
    ...
    await deps.audit.emit({
      ...
      requestId: ctxGet(c as any, 'requestId')
    });
