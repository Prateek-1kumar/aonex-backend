...
export function computeScore(components) {
    const total = components.channelMapping * MAPPING_WEIGHTS.channelMapping +
...
export function resolveApproval(score) {
    if (score >= NO_WARNING_THRESHOLD)
        return { approved: true, mappingMethod: "auto", warning: false };
    if (score >= WARNING_THRESHOLD)
        return { approved: true, mappingMethod: "auto", warning: true };
    if (score >= SUGGESTION_THRESHOLD)
        return { approved: false, mappingMethod: "suggestion", warning: false };
