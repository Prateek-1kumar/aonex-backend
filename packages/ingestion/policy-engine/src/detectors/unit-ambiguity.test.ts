...
describe("detectUnitAmbiguity", () => {
  ...
  it("does not fire when unit is present and convertible", () => {
    const facts = [{ rawKey: "battery_capacity", extractedValue: 5000, unit: "Wh", confidence: 0.8 } as never];
  ...
  it("fires when unit is unconvertible (e.g., mAh without voltage)", () => {
    const facts = [
      { rawKey: "battery_capacity", extractedValue: 5000, unit: "mAh", confidence: 0.8 } as never,
    ];
    const s = detectUnitAmbiguity({ facts, payload: {} as never, domain: "x.com" } as never);
    expect(s).not.toBeNull();
  });
